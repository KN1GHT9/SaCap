...
import pathlib
import json
import utils.generation_eval_utils as generation_eval_utils
import pprint
import warnings
...
def eval_metrics(candidates_json,references_json):
    with open(candidates_json) as f:
        candidates = json.load(f)
    
    image_ids=list(candidates.keys())
    ...
    with open(candidates_json.replace(".json", ".res"), 'w') as f:
        for k, v in other_metrics.items():
            if k == 'bleu':
                for bidx, sc in enumerate(v):
                    print('BLEU-{}: {:.4f}'.format(bidx+1, sc),file=f)
            else:
                print('{}: {:.4f}'.format(k.upper(), v),file=f)
