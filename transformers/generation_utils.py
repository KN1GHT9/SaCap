...
class GenerationMixin:
    ...
    def _expand_inputs_for_generation(
        ...
        prefix: Optional[ModelOutput] = None,
        **model_kwargs,
        ...
        #################
        # add prefix , encoder_hiddend_states , encoder_attention_mask
        if "encoder_hidden_states" in model_kwargs and model_kwargs["encoder_hidden_states"] is not None:
            encoder_hidden_states=model_kwargs["encoder_hidden_states"]
            model_kwargs["encoder_hidden_states"] = encoder_hidden_states.index_select(0, expanded_return_idx)

        if "encoder_attention_mask" in model_kwargs and model_kwargs["encoder_attention_mask"] is not None:
            encoder_attention_mask=model_kwargs["encoder_attention_mask"]
            model_kwargs["encoder_attention_mask"] = encoder_attention_mask.index_select(0, expanded_return_idx)

        if prefix is not None:
            prefix = prefix.index_select(0, expanded_return_idx)

            return input_ids,prefix, model_kwargs
        else:
        ...
        **model_kwargs,
        ...
        if is_greedy_gen_mode:
            ...
            if prefix is None:
                ...
                input_ids,prefix, model_kwargs = self._expand_inputs_for_generation(
                    input_ids, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder,prefix=prefix, **model_kwargs
                )
            # 12. run beam search
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if model_inputs["input_ids"][0][0] in [40290,50257,50256,21231]:
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # change 40290==“prefix”
            if model_inputs["input_ids"][0][0] in [40290,21231 ,50256, 50257]:
            # if model_inputs["input_ids"][0][0] == 40290:
                outputs = self(
                    **model_inputs,
                    return_dict=True,
                    ...
                    prefix=prefix,
                    encoder_attention_mask=model_kwargs["encoder_attention_mask"] if "encoder_attention_mask" in model_kwargs else None,
                    encoder_hidden_states=model_kwargs["encoder_hidden_states"] if "encoder_hidden_states" in model_kwargs else None,
                )
            else:
                outputs = self(
                    **model_inputs,
